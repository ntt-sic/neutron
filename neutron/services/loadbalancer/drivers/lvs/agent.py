...
cfg.CONF.register_opts(lvs_opts, 'lvs')
...
# TODO: better name, device driver abstraction class
class LBaaSL3AgentDriver(object):

    RPC_API_VERSION = '2.0'

    def __init__(self, conf, l3agent):
        LOG.debug(_("Initializing loadbalancer agent"))
        self.use_namespaces = conf.use_namespaces
        self.root_helper = conf.root_helper
        self.l3agent = l3agent

        self.lb_conf_dir = os.path.abspath(
            os.path.normpath(cfg.CONF.lvs.loadbalancer_confs))
        if not os.path.isdir(self.lb_conf_dir):
            os.makedirs(self.lb_conf_dir, 0o755)
        self.lb_state_cache = {}

        self.conn = rpc.create_connection()
        self.conn.create_consumer('%s.%s' % (topics.LBAAS_ON_L3_AGENT,
                                             conf.host),
    ...
    def member_status_update(self, member, updown):
        if updown == 'up':
            ...
            raise ValueError(_('Arg must be up or down'))
        self.lbaas_rpc.update_status('member', member, status)
            ...
            for s in ['up', 'down']:
                buf += ("\tnotify_%s \"%s %s %s %s\"\n" %
                        (s,
                         ...
                         cfg.CONF.lvs.lb_status_relay_socket,
                         m['id'], s))
    ...
    def enable_keepalived(self, pool_id, ns_name, force_reload=False):
        ...
        if pid['main'] is None and pid['checker']:
            utils.execute(['kill', '-15', pid['checker']], self.root_helper)
        elif pid['main'] and pid['checker'] is None:
            utils.execute(['kill', '-15', pid['main']], self.root_helper)
            pid['main'] = None
    ...
    def disable_keepalived(self, pool_id):
        ...
        if checker_pid:
            cmd = ['kill', '-15', checker_pid]
            # race condition can result in ESRCH
            utils.execute(cmd, self.root_helper, check_exit_code=False)
    ...
    def _reload_pool(self, pool_id=None, host=None):
        ...
        ri = self.l3agent.router_info[pool_info['router_id']]
        ...
        for p in ri.internal_ports:
            if p['subnet']['id'] == pool_info['vip']['subnet_id']:
                ifname = self.l3agent.get_external_device_name(p['id'])
                break
        if ifname is None and ri.router['gw_port']:
            if ri.router['gw_port']['subnet']['id'] == \
               pool_info['vip']['subnet_id']:
                ifname = self.l3agent.get_external_device_name(
                    ri.router['gw_port_id'])
        ...
        self.lbaas_rpc.pool_deployed(pool_id)
    ...
    def update_pool(self, context, old_pool, pool):
        LOG.debug(_("update_pool called"))
        self._reload_pool(pool_id=pool)

    def delete_pool(self, context, pool):
        LOG.debug(_("delete_pool called"))

        pool_info = self.lb_state_cache.get(pool)
        if pool_info is None:
            return

        routers = self.l3agent.plugin_rpc.get_routers(
...
class LBMemberStatusRelay(object):
    ...
    def __init__(self, conf, status_update_callback):
        ...
        try:
            os.unlink(cfg.CONF.lvs.lb_status_relay_socket)
        except OSError:
            if os.path.exists(cfg.CONF.lvs.lb_status_relay_socket):
                raise

        dir = os.path.dirname(cfg.CONF.lvs.lb_status_relay_socket)
        try:
            os.rmdir(dir)
        except OSError:
            if os.path.exists(dir):
                raise
        os.mkdir(dir, 0o700)
        listener = eventlet.listen(cfg.CONF.lvs.lb_status_relay_socket,
